                        ...
                        <div class="wk-post-section my-3">
                            ...
                            <div class="row d-flex justify-content-between wk-post-like-section">
                                <div class="col">
                                    ...
                                    <i wire:click="addLike({{$article->id}})" class="{{ $current_like_status ? 'fas' : 'far' }}  fa-heart" style="{{ $current_like_status ? $like_color : '' }}"></i>
                                </div>
                            </div>

                            <div class="row d-flex flex-column wk-post-tags-section">
                                <div class="col tag-title">بر چسب ها :</div>
                                <div class="col wk-post-tag">
                                    @foreach($article->tags as $tag)
                                        <span class="wk-post-tag-name"><a
                                                href="{{ route('articlesByTag',[$tag]) }}">{{ $tag->title_persian }}</a></span>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        <div class="row d-flex justify-content-center write-comments-section my-4">
                            <div class="col-xxl-10 col-xl-9 col-lg-9 col-md-9 col-10">
                                <form wire:submit.prevent="submit">
                                    ...
                                    @else
                                        <div class="mb-3">
                                            <a href="{{ route('registerForm') }}" class="btn btn-outline-primary">
                                                برای ارسال
                                                دیگاه ابتدا
                                                وارد سایت
                        ...
                        <div class="row my-5 list-comments-section d-flex justify-content-center align-items-center">
                            @if ($article->comments->where('article_id', '=', $article->id)->where('approved','=',1))
                                @foreach ($article->comments->where('approved',1) as $comment)
                                    <div class="col-xxl-11 col-xl-9 col-lg-9 col-md-9 col-10">
                                        <div class="d-flex flex-start mb-4">
                                            <img class="rounded-circle shadow-1-strong me-3"
                                                 src="{{ $comment->user->image_path
                                    ? asset('images/users/' . $comment->user->image_path)
