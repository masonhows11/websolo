                    ...
                    <div class="col">
                        <div class="card">
                            <img src="{{ asset('storage/samples/'.$sample->main_image) }}"
                                 ...
                                 alt="sample-image">
                            <div class="card-body comment-post-title">
                                <h5 calss="card-title">{{ $sample->title_persian }} - {{ $sample->title_english }}</h5>
                ...
                <div class="accordion" id="accordionExample">
                    <div class="accordion-item mt-5">
                        <div class="accordion-header border border-2" id="headingOne">
                            <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $sample->id }}" aria-expanded="true" aria-controls="collapseOne">
                            </button>
                        </div>
                        <div id="collapse-{{ $sample->id }}" class="accordion-collapse collapse {{ $sample->comments->count() >= 1 ? 'show' : '' }} " aria-labelledby="headingOne" data-bs-parent="#accordionExample">
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '#approved_comment', function (event) {
                ...
                $.ajax({
                    method: 'GET',
                    url: '{{ route('admin.sampleApproveComment') }}',
                    data: {id:comment_id},
                }).done(function (data) {
                    console.log(data);
                    if (data['status'] === 404) {
            ...
            $(document).on('click', '#delete_comment', function (event) {
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            method: 'GET',
                            url: '{{ route('admin.sampleDeleteComment') }}',
                            data: {id:comment_id},
                        }).done(function (data) {
                            if (data['status'] === 404) {
                            ...
                            if (data['status'] === 500) {swal.fire({icon: 'error', text: data['message'],})
                            }
                        })
                    }
                })
